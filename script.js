let clock = () => {
    ...
    if (period === "AM" && 1 <= hours && hours <= 12) {
        ...
        if (period === "PM" && (hours === 12 || (1 <= hours && hours <= 3))) {
            ...
            image.src = "./images/Group 5183.jpg";
...
const setAlarm = () => {
    ...
    let wakeUpTime = selectMorningTime.options[selectMorningTime.selectedIndex].value;
    let lunchTime = selectNoonTime.options[selectNoonTime.selectedIndex].value;
    let napTime = selectEveningTime.options[selectEveningTime.selectedIndex].value;
